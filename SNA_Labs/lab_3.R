...
# For example, to get vertices reachable from node 1 via outgoing links:
subcomponent(krack_advice, 1, mode = 'out') %>% sort()
# There are outgoing paths from Node 1 to all other nodes; so, actor 1
# can pass a message to any other network actor
 
# Note: the subcomponent function returns a vector of ids
# of vertices that are in the same component as the given vertex;
# so, the result includes the vertex itself.

# Let's see about incoming links:
subcomponent(krack_advice, 1, mode = 'in') %>% sort()
# Also, any other actor can reach actor 1 through advice connections

# Let's see how connected this actor is in another network
subcomponent(krack_reports_to, 1, mode = 'out')
subcomponent(krack_reports_to, 1, mode = 'in')
# None can reach actor 1, while he/she can reach 2 other actors through
# outgoing links
...
# both through incoming and outgoing edges.
...
# reachable by all other nodes through incoming links
...
# add 'arrows' attribute to display edge direction:
...
# Average reachability for a network reveals what percent of the network 
...
max_reachability <- vcount(krack_reports_to)*(vcount(krack_reports_to)-1)
sum(reach_reports_to_in)/max_reachability
sum(reach_reports_to_out)/max_reachability
# Only ~8.6% of the potential pairwise connections were realized
...
# This is done by calculating geodesics, or shortest paths between
...
# First, let's compute shortest paths to each vertex (from all other vertices)
...
# Next, shortest paths from each vertex (to all other vertices)
...
# Let's visualize the distribution of shortest paths to get a better picture
# First, prepare the data for shortest paths via incoming links
data.frame(sp_friendship_in) %>% 
  mutate(ID = factor(1:21)) %>%
  pivot_longer(cols = starts_with("X"),
               names_to = "to",
               values_to = "SP") -> sp_friend_in_long
# Then, make a plot (boxplot)
ggplot(data = sp_friend_in_long,
       mapping = aes(x = ID, y = SP, group=ID, fill=ID)) +
  geom_boxplot(show.legend = FALSE) +
  # geom_jitter(show.legend = FALSE, size=0.75) +
  labs(x = 'Node ID', y = "Shortest Path") +
...
#
# TASK 2:
# Try to create the same kind of graph for the distribution of 
# shortest paths via outgoing links
#
##
...
# shortest paths (~2), suggests that we have some peripheral nodes in the network. 
...
# Get the shortest path (sequence of edges) that connects the two vertices
# that are most apart:
?shortest_paths
...
# Which of these two we will use depend on the task we are
# dealing with, that is, the questions we aim to address by
# doing the analysis
...
# A node has coreness k if it belongs to a k-core but not to (k+1)-core.
...
compute_assortitivity <- function(g) {
  ...
  result$level <- assortativity_nominal(g, types = as.factor(V(g)$LEVEL))
  result$degree <- assortativity_degree(g)
  return(result)
}
# Note that for the DEPT and LEVEL attributes, assortativity is computed 
# in a different way - using assortativity_nominal() f. - since
# these attributes are, in essence, nominal
...
# NOTE: this function assumes that the categorical attribute has been 
...
# In this data set, we have two nominal attributes: department and level.
# Let's look at homophily across departments.
# As it is already coded 0-4, no recoding is needed. 
...
#
# TASK 6: 
# Compute the heterogeneity with respect to the 2nd nominal attribute - level.
# Note that it should be 'recoded', so that the minimal value is 0.
# 
